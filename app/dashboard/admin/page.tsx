...
const mutedPanelClass = "rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] p-4";
const inputClass = "min-h-12 w-full rounded-2xl border border-[var(--border)] bg-[var(--surface)] px-4";
...
export default function AdminPage() {
  ...
  async function handleSaveUser() {
    ...
    setSavingUser(true);
    const { data, error } = await updateUserForAdmin(selectedUser.id, {
    ...
    setSavingUser(false);
    if (error) {
      setMessage(error.message || "Kullanıcı güncellenemedi.");
      return;
    }
    if (data) setSelectedUser(data as UserRow);
  ...
  async function handleToggleBan() {
    ...
    const { data, error } = await updateUserForAdmin(selectedUser.id, { is_active: nextActive });
    setSavingUser(false);
    if (error) {
    ...
    if (data) {
      setSelectedUser(data as UserRow);
    } else {
              ...
              <>
                <section className={`mt-5 ${mutedPanelClass}`}>
                  ...
                  <div className="mt-3 space-y-3">
                    {pending.length === 0 ? (
                      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-sm text-[var(--foreground-muted)]">Bekleyen soru yok.</div>
                    ) : (
                ...
                <section className={`mt-5 ${mutedPanelClass}`}>
                  ...
                  <div className="mt-3 space-y-3">
                    {filteredQuestions.length === 0 ? (
                      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-sm text-[var(--foreground-muted)]">Bu filtrede soru bulunamadı.</div>
                    ) : (
              ...
              <section className={`mt-5 ${mutedPanelClass}`}>
                ...
                <div className="mt-3 space-y-3">
                  {filteredUsers.length === 0 ? (
                    <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 text-sm text-[var(--foreground-muted)]">Kullanıcı bulunamadı.</div>
                  ) : (
            ...
            <div className={panelClass}>
              {selectedUser ? (
                <div className="space-y-4">
                  ...
                  <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Ad Soyad</label><input value={selectedUser.name} onChange={(e) => setSelectedUser({ ...selectedUser, name: e.target.value })} className={inputClass} /></div>
                  <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">E-posta</label><input value={selectedUser.email ?? ""} onChange={(e) => setSelectedUser({ ...selectedUser, email: e.target.value })} className={inputClass} /></div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Rol</label><select value={selectedUser.role} onChange={(e) => setSelectedUser({ ...selectedUser, role: e.target.value as UserRow["role"] })} className={inputClass}><option value="student">Öğrenci</option><option value="admin">Admin</option></select></div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Kullanıcı ID</label><div className="break-all rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-3 text-sm text-[var(--foreground-muted)]">{selectedUser.id}</div></div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={handleToggleBan} disabled={savingUser} className={`rounded-2xl px-4 py-2 text-sm font-semibold text-white disabled:opacity-70 ${selectedUser.is_active ? "bg-rose-600" : "bg-emerald-600"}`}>{selectedUser.is_active ? "Üyeyi Banla" : "Banı Kaldır"}</button>
                  </div>
                </div>
              ) : selectedQuestion ? (
                <div className="space-y-4">
                  <h2 className="text-lg font-semibold text-[var(--foreground)]">Soru Detayı</h2>
                  <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Konu</label><select value={selectedQuestion.topic_id} onChange={(e) => setSelectedQuestion({ ...selectedQuestion, topic_id: Number(e.target.value) })} className={inputClass}>{topics.map((topic) => <option key={topic.id} value={topic.id}>{topic.title}</option>)}</select></div>
                  ...
                  {optionKeys.map((option) => <div key={option.key}><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">{option.label} Şıkkı</label><input value={selectedQuestion[option.key]} onChange={(e) => setSelectedQuestion({ ...selectedQuestion, [option.key]: e.target.value })} className={inputClass} /></div>)}
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Doğru Şık</label><select value={selectedQuestion.correct_option} onChange={(e) => setSelectedQuestion({ ...selectedQuestion, correct_option: e.target.value as QuestionRow["correct_option"] })} className={inputClass}><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select></div>
                    <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Durum</label><select value={selectedQuestion.status} onChange={(e) => setSelectedQuestion({ ...selectedQuestion, status: e.target.value as QuestionStatus })} className={inputClass}><option value="pending">Beklemede</option><option value="approved">Onaylandı</option><option value="rejected">Reddedildi</option></select></div>
                  </div>
                  <div><label className="mb-1 block text-sm font-medium text-[var(--foreground)]">Hazırlayan</label><input value={selectedQuestion.created_by_name} onChange={(e) => setSelectedQuestion({ ...selectedQuestion, created_by_name: e.target.value })} className={inputClass} /></div>
